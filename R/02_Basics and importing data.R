...
# By: <insert your name> (e.g. Vaughn DiMarco)
# Description: Introduction to R workshop presented by CIEE
# Version of R used: RStudio 0.98.310
...
# 1. HOUSEKEEPING ############################################################
# Functions that will ensure that your script runs smoothly.
...
# setwd("/path/to/folder/Introduction_to_R-project_folder-master")
...
iris_data<-read.csv("Data/iris_broken.csv")
# Error 1: wrong extension (.txt not .csv)

iris_data<-read.csv("Data/iris_broken.txt")
...
iris_data<-read.csv("Data/iris_broken.txt", sep = "")
...
iris_data<-read.csv("Data/iris_broken.txt", sep = "", skip = 4)
...
iris_data<-read.csv("Data/iris_broken.txt", sep = "", skip = 4, na.strings = c("NA","na","Forgot_this_value"))
                      
head(iris_data)
str(iris_data)

# Error 5: Numerous variables are not numerical
class(iris_data$Sepal.Length)  # numeric  YAY!
class(iris_data$Sepal.Width)   # factor   ;(
class(iris_data$Petal.Length)  # factor   ;(
class(iris_data$Petal.Width)   # numeric  YAY!
...
iris_data<-read.csv("Data/iris_broken.txt",
                      ...
                      as.is=c("Sepal.Width", "Petal.Length")) # tells R to load as is

head(iris_data)
str(iris_data)

# that was not perfect, now R thinks these should only be characters, 
# not numeric values use "as.class wanted(argument)" where you can 
# replace "class wanted"" with "numeric", "ordered" or "factor"              
...
# a. Saving an R file 
save(iris_data, file = "Data/iris_cleaned.R")               

# Clear your memory
rm(list = ls())
  
# b. reload iris_data
load("Data/iris_cleaned.R")
...
# or, equivalently:
iris_data[ , c("Sepal.Length", "Sepal.Width","Petal.Length","Petal.Width")] <-sapply(
  iris_data[,c("Sepal.Length",
               "Sepal.Width",
               "Petal.Length",
               "Petal.Width")],scale)
...
iris_data<-read.csv("Data/iris_good.csv")  # reload the data iris 

normalized_iris<-data.frame(iris_data["Species"],
                            sapply(iris_data[,c("Sepal.Length",
                                               "Sepal.Width",
                                               "Petal.Length",
                                               "Petal.Width")],scale))
...
write.table(normalized_iris, file="Data/normalized_iris.csv", sep = ",")
...
if(!require(foreign)){    #check if package is installed
  install.packages("foreign")  #install package if it is not
  require(foreign)}  #load package after install

#Same as above, but without checking existing installation
...
# 8. DEALING WITH THE BROKEN WITH THE BROKEN CO2 DATA! #######################
